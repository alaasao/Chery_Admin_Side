...
const TicketCard: FC<TicketCardProps> = ({ id, Name,Subject }: TicketCardProps) => {
  ...
  >
    ...
    <div className="col-span-3 truncate max-md:hidden">
      {" "}
      <div className="truncate max-md:hidden ">{Subject}</div>
    </div>
    ...
    <Link to={`/ticket/editticket/${id}`}>  <LuPencil className="text-[#494545] cursor-pointer"   /></Link>
    <DelButt id={id || ""} deleteRoute="ticket" icon={true} back="/ticket" name="ticket" />
